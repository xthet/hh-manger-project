	...
	: describe("Campaign Unit Tests", function () {
			...
			describe("donate", function () {
				it("reverts if creator tries to donate", async () => {
					...
					await expect(
						campaign.donate(deployer, false, { value: donationAmount })
					).to.be.reverted
				})
				...
				//   const donateTx = await donatorCampaign.donate(donator, false, { value: donationAmount })
			...
			describe("checkUpkeep", function () {
				it("emits campaign successful if goal is reached", async () => {
					...
					const donateTx = await donatorCampaign.donate(donator, false, {
			...
			describe("refund", function () {
				...
				it("successful if caller has donations", async () => {
					...
					const donateTx = await donatorCampaign.donate(donator, true, {
						value: donationAmount,
					})
					// here donationAmount was 5 eth
					const donateTxR = await donateTx.wait(1)
			...
			describe("make reward", function () {
				it("successfully creates rewards", async () => {
					...
					const rewardDets = await campaign.getReward(
